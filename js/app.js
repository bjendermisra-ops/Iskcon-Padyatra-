let appTheme = 'light'; // Default Day Mode selected on application startup
let mockupHistory = ['m-home']; // Track simulated iphone tabs for true back button navigation flow
...
// Mock Phone Screen Toggling with full state history tracking
function switchMockup(screenId, btnElement, isBackAction = false) {
    ...
    // Highlight correct corresponding navigation tab button in mockup
    const activeTabBtn = document.getElementById(`btn-${screenId}`);
    if (activeTabBtn) {
    ...
    // Append state history if navigating forward
    if (!isBackAction) {
        if (mockupHistory[mockupHistory.length - 1] !== screenId) {
            mockupHistory.push(screenId);
        }
    }

    // Toggle mock back arrow button visibility dynamically
    const backBtn = document.getElementById('mock-back-btn');
    if (backBtn) {
...
// True Simulated Back Navigation Trigger for Mock phone frame
function mockupBack() {
    if (mockupHistory.length > 1) {
        mockupHistory.pop(); // Pop current active screen state
        const previousScreen = mockupHistory[mockupHistory.length - 1];
        switchMockup(previousScreen, null, true);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Explicitly set Day Mode default theme configurations on bootup
